...
from NerveSegmentation.slim import losses
from NerveSegmentation.slim import scopes
from NerveSegmentation.slim import variables
...
def prelu(inputs,
    ...
    :param is_training: whether or not the model is in training mode.
    :param trainable: whether or not the variables should be trainable or not.
    :param restore: whether or not the variables should be marked for restore.
    :param scope: Optional scope for variable_op_scope.
    ...
    with tf.variable_op_scope([inputs], scope, 'PReLU', reuse=reuse):
        params_shape = inputs_shape[-1:]
        alpha = variables.variable('alpha',
                                   params_shape,
                                   initializer=tf.zeros_initializer,
                                   trainable=trainable,
                                   restore=restore)
        outputs = tf.nn.relu(inputs) + tf.mul(alpha, (inputs - tf.abs(inputs))) * 0.5
...
def batch_norm(inputs,
    ...
    Args:
      ...
      prelu_activation: apply prelu activation function
    ...
    with tf.variable_op_scope([inputs], scope, 'BatchNorm', reuse=reuse):
        ...
        outputs.set_shape(inputs.get_shape())
        if prelu_activation is not None:
            with scopes.arg_scope([prelu], trainable=trainable, restore=restore):
...
def atrous_conv2d(inputs,
                  ...
                  scope=None,
                  reuse=None):
    """Adds a 2D atrous convolution followed by an optional batch_norm layer.

    atrous conv2d creates a variable called 'weights', representing the convolutional
    kernel, that is convolved with the input. If `batch_norm_params` is None, a
    second variable called 'biases' is added to the result of the convolution
    operation.
    ...
    """
    with tf.variable_op_scope([inputs], scope, 'Atrous_Conv', reuse=reuse):
        kernel_h, kernel_w = _two_element_tuple(kernel_size)
        stride_h, stride_w = _two_element_tuple(stride)
        num_filters_in = inputs.get_shape()[-1]
        weights_shape = [kernel_h, kernel_w,
        ...
        weights = variables.variable('weights',
                                     ...
                                     trainable=trainable,
                                     restore=restore)
        conv = tf.nn.atrous_conv2d(inputs, weights, rate, [1, stride_h, stride_w, 1],
                                   padding=padding)
        if batch_norm_params is not None:
            with scopes.arg_scope([batch_norm], is_training=is_training,
                                  trainable=trainable, restore=restore):
...
def conv2d_transpose(inputs,
                     ...
                     scope=None,
                     reuse=None):
    """Adds a 2D transpose convolution followed by an optional batch_norm layer.

    conv2d transpose creates a variable called 'weights', representing the convolutional
    kernel, that is convolved with the input. If `batch_norm_params` is None, a
    second variable called 'biases' is added to the result of the convolution
    operation.
    ...
    """
    with tf.variable_op_scope([inputs], scope, 'Conv_Trans', reuse=reuse):
        kernel_h, kernel_w = _two_element_tuple(kernel_size)
        stride_h, stride_w = _two_element_tuple(stride)
        num_filters_in = inputs.get_shape()[-1]
        ...
        output_shape = tf.pack([N, H*2, W*2, num_filters_out])
        conv = tf.nn.conv2d_transpose(inputs, weights, output_shape, [1, stride_h, stride_w, 1],
                                      padding=padding)
        if batch_norm_params is not None:
            with scopes.arg_scope([batch_norm], is_training=is_training,
                                  trainable=trainable, restore=restore):
